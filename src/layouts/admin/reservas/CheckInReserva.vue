  ...
  <v-container>
    <v-row>
      ...
      <v-col cols="6">
        ...
        <v-container>
          <v-card elevation="7">
            <v-btn block prepend-icon="mdi-account-check" append-icon="mdi-chevron-right" color="primary"
              variant="elevated" class="text-success" @click="checkIn">check in - habitacion</v-btn>
          </v-card>
        </v-container>
      </v-col>
    </v-row>
...
export default {
  ...
  methods: {
    ...
    async checkIn() {
      try {
        this.$swal({
          title: "¿Estas seguro que quieres realizar el CHECK IN?",
          icon: 'question',
          showCancelButton: true,
          confirmButtonText: "Si, quiero",
        }).then((result) => {
          if (result.isConfirmed) {
            if (this.reserva.habitacion.estado_habitacion == 1) {
              this.reserva.estado_reserva = 5
              this.reserva.condicion_reserva = 1
              this.reserva.habitacion.estado_habitacion = 2
              this.reserva.hora_llegada_reserva = moment().format('HH:mm')
              ...
              this.$nextTick(async () => {
                ...
                setTimeout(async () => {
                  ...
                  if (resp.status == 200) {
                    ...
                    if (res.status == 200) {
                      this.$swal({
                        title: "CHECK IN - HABITACION!",
                        text: `se ha realizado el check in de la habitacion CORRECTAMENTE!!!`,
                        icon: "success",
                        timer: 2500,
                        didClose: () => {
                          this.overlay = false
              ...
              this.$swal({
                title: "Error!",
                text: "La Habitacion no esta VACANTE. por favor revise y realize su disponibilidad!",
                icon: "error",
                timer: 4500
              })
            }
