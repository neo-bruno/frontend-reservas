        ...
        <v-card>
          <v-card-title class="bg-primary">
            <div class="d-flex justify-space-between">
              <span class="font-weight-bold text-success"> <v-icon>mdi-plus</v-icon> Añadir Categoria </span>
          ...
          <v-card-text>
            <v-form ref="formulario" @submit.prevent="">
              <v-container>
                ...
                <v-row>
                  <v-col cols="6">
                    <p class="pt-3" style="font-size: 1rem;">precio actual (*)</p>
                  </v-col>
                  <v-col cols="6">
                ...
                <v-row>
                  ...
                  <v-col cols="6">
                    <v-text-field ref="precio_anterior" type="number" v-model="categoria.precio_antes_categoria"
                      ...
                      :rules="[rules.required, rules.soloNumeroODecimal]" suffix="Bs.-"
                      @keyup.enter.prevent="setfocus('descuento')"></v-text-field>
                  </v-col>
                </v-row>
                ...
                <v-row justify="end">
                  <v-col cols="auto" class="d-flex justify-end">
                    <v-btn color="success" block prepend-icon="mdi-content-save" append-icon="mdi-chevron-right"
                      @click="guardarDatos">guardar datos categoria</v-btn>
                  </v-col>
                </v-row>
              </v-container>
            </v-form>
...
<script>
import { saveCategory } from '@/services/categoria.service';
        ...
        this.$nextTick(async () => {
          ...
          if (valid) {
            setTimeout(async () => {
              ...
              let res = await saveCategory(this.categoria)
              if (res.status == 200) {
                this.$swal({
                  title: "Registro Guardado!",
                  text: `se ha guardado los datos de la categoria CORRECTAMENTE!!!`,
                  icon: "success",
                  timer: 2500,
                  didClose: () => {
                    this.overlay = false
