      ...
      <v-col cols="12">
        <v-card>
          ...
          <v-card-text>
            ...
            <v-row>
              <v-col cols="7">
                <v-select v-model="habitacion.nivel.id_nivel" color="success" :base-color="habitacion.nivel.id_nivel > 0 ? 'success' : 'primary'" clearable label="Elejir Nivel de Piso" :items="items"
                  @update:model-value="handleLevelChange" variant="outlined"></v-select>
              </v-col>
              <v-col cols="1">
                <v-icon :color="habitacion.nivel.id_nivel > 0 ? 'success' : 'cero'" size="50">mdi-check</v-icon>
              </v-col>
            </v-row>
            ...
            <v-row>
              <v-col cols="7">
                <v-select v-model="habitacion.categoria.id_categoria" color="success" :base-color="habitacion.categoria.id_categoria > 0 ? 'success' : 'primary'" label="Eleja una Categoria" :items="itemsCategoria" variant="outlined"
              ...
              <v-col cols="2">
                <v-text-field label="precio ahora" class="text-success font-weight-bold" readonly
                  prepend-icon="mdi-cash" :model-value="habitacion.categoria.precio_ahora_categoria" suffix="Bs.-"
                  variant="underlined"></v-text-field>
              </v-col>
              <v-col cols="2">
                <v-text-field label="precio antes" class="text-decoration-line-through text-error" readonly
                  prepend-icon="mdi-cash" :model-value="habitacion.categoria.precio_antes_categoria" suffix="Bs.-"
                  variant="underlined"></v-text-field>
              </v-col>
              <v-col cols="1">
                <v-icon :color="habitacion.categoria.id_categoria > 0 ? 'success' : 'cero'" size="50">mdi-check</v-icon>
              </v-col>
            </v-row>
            ...
            <v-row>
              <v-col cols="12" class="pt-0">
                ...
                <v-table hover density="compact">
                  ...
                  <tbody>
                    <tr v-for="(habitacion_cama, index) in habitacion.habitacion_camas" :key="index">
  ...
  <v-dialog v-model="dialogo_editar_cama" width="600" persistent>
    <v-form ref="editar" @submit.prevent>
      <v-card rounded="lg">
        ...
        <v-card-title class="text-septary text-center">
          <div class="text-h4 text-success">
            {{ getFirstWord('Modificar') }}
            <span class="text-primary">{{ getSecondtWord(' Camas de la Habitacion') }}</span>
          </div>
        </v-card-title>
        <v-card-subtitle>
          <div class="text-center text-wrap">elija la(s) cama(s) que pertenece a la categoria. y ponga el precio de la
        ...
        <v-card-actions v-if="paso == 2 || !nuevo" class="mt-4">
          <v-btn v-if="nuevo" text="atras" color="septary" prepend-icon="mdi-chevron-left" variant="tonal"
            @click="paso--"></v-btn>
          <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    ...
    handleCategoriaChange(id) {
      ...
      if (!categoria) return
      this.habitacion.categoria = categoria
      this.habitacion = {
        ...this.habitacion,
        categoria: {
    ...
    async agregarCamaCosto(cama, cantidad) {
      try {
        ...
        this.habitacion.habitacion_camas.push(habitacion_cama)
    ...
    mostrarDialogoCamaNuevo() {
      this.nuevo = true
      this.habitacion.habitacion_camas.forEach(ele => {
    ...
    async modificarCamaCosto(cama, cantidad) {
      try {
        ...
        const index = this.habitacion.habitacion_camas.findIndex(
        ...
        if (index !== -1) {
          ...
          this.habitacion.habitacion_camas[index] = {
            ...this.habitacion.habitacion_camas[index],
            cantidad_hab_cama: cantidad,
            costo_hab_cama: cama.costo_cama,
            total_hab_cama: total.toFixed(2)
        ...
        console.log(this.habitacion.habitacion_camas[index])
        ...
        this.habitacion.habitacion_camas = camasActualizadas
    ...
    eliminarCama(data) {
      this.$swal({
        ...
        this.$swal({
          ...
          this.habitacion.habitacion_camas = this.habitacion.habitacion_camas.filter(item => item.cama.id_cama != data.cama.id_cama)
          this.obtenerCamas()
        })
      })
    },
