    ...
    <v-row>
      ...
      <v-col cols="4" class="pt-0">
        <v-list select-strategy="single-independent" lines="two" rounded>
          ...
          <v-list-item color="octary" v-for="(servicio, index) in habitacion.servicios" :key="servicio.id_servicio"
      ...
      <v-col cols="8">
        <v-row>
          ...
          <v-col cols="8">
            <draggable v-model="habitacion.imagenes" item-key="url_imagen" tag="div" class="imagenes-grid" :animation="500"
                ...
                <div v-if="index < 6" class="img-item">
                  ...
                  <v-card v-if="index === 5" elevation="7">
                    <v-img :src="element.preview || element.url_imagen" aspect-ratio="1" class="cursor-grab">
                      ...
                      <v-btn v-if="habitacion.imagenes.length > 6" size="small" elevation="2" color="primary" class="ver-mas-btn"
                        @click="showGaleryImages()">
                        <v-icon start>mdi-image-multiple</v-icon>
                        Ver {{ habitacion.imagenes.length - 6 }} más
                      </v-btn>
                    </v-img>
                  </v-card>
  ...
  <v-dialog v-model="dialogo_galeria" fullscreen transition="dialog-bottom-transition">
    <template #default>
      ...
      <div class="galeria-container">
        <v-container fluid>
          <v-row dense>
            <draggable v-model="habitacion.imagenes" item-key="url_imagen" tag="div" :animation="300" ghost-class="dragging"
              class="d-flex flex-wrap w-100">
              <template #item="{ element, index }">
                <v-col cols="12" sm="6" md="4" lg="3" xl="2" class="pa-2">
                  <v-card class="overflow-hidden">
                    <v-img :src="element.preview || element.url_imagen" height="200" class="cursor-grab">
...
export default {
  ...
  methods: {
    ...
    async onFilesChange() {
      ...
      this.habitacion.imagenes.push(...nuevasImagenes)
    ...
    async removeImage(index,) {
      try {
        this.habitacion.imagenes.splice(index, 1)
    ...
    abrirDialogoServicio() {
      this.habitacion.servicios.forEach(item => {
    ...
    seleccionoServicio(item) {
      this.habitacion.servicios.push(item)

      const existe = this.habitacion.servicios.some(
    ...
    eliminarServicio(servicio) {
      this.habitacion.servicios = this.habitacion.servicios.filter(item => item.id_servicio != servicio.id_servicio)
      // this.habitacion = {
      //   ...this.habitacion,
