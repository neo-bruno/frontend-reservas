    ...
    <v-stepper color="primary" v-model="step" next-text="siguiente" prev-text="atras">
      ...
      <v-stepper-window class="ma-0">
        <v-stepper-window-item :value="1">
          <EditarPaso1 v-model="habitacion" />
        </v-stepper-window-item>

        <v-stepper-window-item :value="2">
          <EditarPaso2 v-model="habitacion" />
        </v-stepper-window-item>

        <v-stepper-window-item :value="3">
          <EditarPaso3 v-model="habitacion" />
        </v-stepper-window-item>
...
import EditarPaso1 from './pasos/EditarPaso1.vue';
import EditarPaso2 from './pasos/EditarPaso2.vue';
import EditarPaso3 from './pasos/EditarPaso3.vue';
import { getRoomById, modifyRoom } from '@/services/habitacion.service';
...
export default {
  ...
  computed: {
    ...
    habitacionCompleta() {
      ...
      return (
        h.categoria.id_categoria > 0 &&
        h.nivel.id_nivel > 0 &&
        h.habitacion_camas.length > 0 &&
        h.numero_habitacion &&
        h.nombre_habitacion &&
  ...
  methods: {
    ...
    siguiente() {
      if (this.step == 1 && this.habitacion.habitacion_camas.length > 0 && this.habitacion.nivel.id_nivel > 0 && this.habitacion.categoria.id_categoria > 0) {
        ...
        if (this.step == 2 && this.habitacion.servicios.length > 0 && this.habitacion.imagenes.length > 0) {
          this.step++
        } else {
    ...
    async finalizar() {
      ...
      try {
        ...
        const res = await modifyRoom(this.habitacion)
