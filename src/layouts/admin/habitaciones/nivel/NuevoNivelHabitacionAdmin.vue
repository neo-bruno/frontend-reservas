        ...
        <v-card>
          <v-card-title class="bg-primary">
            <div class="d-flex justify-space-between">
              <span class="font-weight-bold text-success"> <v-icon>mdi-plus</v-icon> Añadir Nivel </span>
                ...
                <v-row justify="end">
                  <v-col cols="7" class="d-flex justify-end">
                    <v-btn color="success" prepend-icon="mdi-content-save" append-icon="mdi-chevron-right"
                      @click="guardarDatos">guardar datos
                      nivel</v-btn>
                  </v-col>
                </v-row>
              </v-container>
...
<script>
import { saveLevel } from '@/services/nivel.service';
        ...
        this.$nextTick(async () => {
          ...
          if (valid) {
            setTimeout(async () => {
              ...
              let res = await saveLevel(this.nivel)
              if (res.status == 200) {
                this.$swal({
                  title: "Registro Guardado!",
                  text: `se ha guardado los datos del nivel CORRECTAMENTE!!!`,
                  icon: "success",
                  timer: 2500,
                  didClose: () => {
                    this.overlay = false
