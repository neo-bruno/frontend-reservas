        ...
        <v-card>
          <v-card-title class="bg-primary">
            <div class="d-flex justify-space-between">
              <span class="font-weight-bold text-warning"> <v-icon>mdi-pencil</v-icon> Modificar Servicio </span>
            ...
            <v-form ref="formulario" @submit.prevent="">
              <v-container>
                ...
                <v-row>
                  ...
                  <v-col cols="5">
                    <v-text-field ref="icono_servicio" v-model="servicio.icono_servicio" density="compact"
                      placeholder="icono" prepend-inner-icon="mdi-rename-box" variant="outlined"
                ...
                <v-row justify="end">
                  <v-col cols="auto" class="d-flex justify-end">
                    <v-btn color="success" block prepend-icon="mdi-content-save" append-icon="mdi-chevron-right"
                      @click="guardarDatos">modificar datos servicio</v-btn>
                  </v-col>
                </v-row>
...
<script>
import { getService, modifyService } from '@/services/servicio.service';

export default {
  data() {
    return {
      id_servicio: this.$route.params.id_servicio,
      servicio: {
        nombre_servicio: '',
        icono_servicio: ''
      },
  ...
  methods: {
        ...
        this.$nextTick(async () => {
          ...
          if (valid) {
            setTimeout(async () => {
              ...
              let res = await modifyService(this.servicio)
              if (res.status == 200) {
                this.$swal({
                  title: "Datos Modificados!",
                  text: `se ha modificado los datos del servicio CORRECTAMENTE!!!`,
                  icon: "success",
                  timer: 2500,
                  didClose: () => {
                    this.overlay = false
    ...
    async obtenerServicio() {
      try {
        let res = await getService(this.id_servicio)
        if (res.status == 201)
          this.servicio = res.data.data
      } catch (error) {
        console.log(error)
      }
    },
