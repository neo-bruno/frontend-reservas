<template>
  <v-card class="pa-4" elevation="2">
    ...
    <div class="d-flex justify-space-between align-center mb-4">
      <h3 class="text-h6 text-error font-weight-regular">Servicios del Hotel</h3>
    ...
    <v-table class="tabla-custom" density="compact" hover>
      ...
      <tbody>
        <tr v-for="(item, i) in paginatedItems" :key="item.id_nivel" class="text-center">
          ...
          <td class="text-left">{{ item.nombre_servicio }}</td>
          <td class="text-left"><v-icon size="30">mdi-{{ item.icono_servicio }}</v-icon></td>
...
export default {
  ...
  computed: {
    datosFiltrados() {
      ...
      return this.datosOriginales.filter((d) =>
        d.nombre_servicio.toLowerCase().includes(this.search.toLowerCase()) ||
        d.icono_servicio.toLowerCase().includes(this.search.toLowerCase())
      );
    },
  ...
  methods: {
    agregarNuevo() {
      this.$router.push({ path: 'servicios/nuevo' })
    },

    editar(item) {
      this.$router.push({
        name: 'editar-servicios-admin',
        params: { id_servicio: item.id_servicio }
      })
    },
